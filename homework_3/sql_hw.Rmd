...
```{r}
dbGetQuery(mydb, "SELECT * FROM Users order by -registered limit 5;")
...
```{r}
dbGetQuery(mydb, "SELECT Artists.name, Albums.name, duration as mean FROM Songs join Albums join Artists
                  where Albums.artist_id = Artists.id AND Songs.album_id = Albums.id
                  group by Songs.album_id
                  order by mean desc
                  limit 1")
